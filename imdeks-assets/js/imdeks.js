...
(function(){
    var weatherCodeMap={0:'Açık',1:'Genelde açık',2:'Parçalı bulutlu',3:'Kapalı',45:'Sisli',48:'Kırağılı sis',51:'Hafif çisenti',53:'Çisenti',55:'Yoğun çisenti',61:'Hafif yağmur',63:'Yağmur',65:'Şiddetli yağmur',71:'Hafif kar',73:'Kar',75:'Yoğun kar',80:'Hafif sağanak',81:'Sağanak',82:'Şiddetli sağanak',95:'Gök gürültülü',96:'Dolu yağışlı',99:'Şiddetli dolu'};
    function text(id,value){var el=document.getElementById(id);if(el)el.textContent=value;}
    function fmtRate(value){var n=Number(value);if(!isFinite(n))return'--';return n.toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:4});}
    function loadWeather(lat,lon,label){text('imdeksWeatherLocation',label||'Konumunuza göre');var url='https://api.open-meteo.com/v1/forecast?latitude='+encodeURIComponent(lat)+'&longitude='+encodeURIComponent(lon)+'&current=temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,weather_code&timezone=auto';fetch(url).then(function(r){return r.json();}).then(function(data){var c=data&&data.current?data.current:{};text('imdeksWeatherTemp',Math.round(c.temperature_2m)+'°');text('imdeksWeatherDesc',weatherCodeMap[c.weather_code]||'Güncel bilgi');text('imdeksFeelsLike',Math.round(c.apparent_temperature)+'°');text('imdeksHumidity',(c.relative_humidity_2m||'--')+'%');text('imdeksWind',Math.round(c.wind_speed_10m||0)+' km/s');}).catch(function(){text('imdeksWeatherLocation','Hava durumu alınamadı');text('imdeksWeatherDesc','Tekrar deneyin');});}
    function detectWeather(){if(navigator.geolocation){navigator.geolocation.getCurrentPosition(function(pos){loadWeather(pos.coords.latitude,pos.coords.longitude,'Konumunuza göre');},function(){loadWeather(41.0082,28.9784,'İstanbul varsayılan');},{enableHighAccuracy:false,timeout:6000,maximumAge:900000});}else{loadWeather(41.0082,28.9784,'İstanbul varsayılan');}}
    function loadCurrency(){fetch('https://open.er-api.com/v6/latest/USD').then(function(r){return r.json();}).then(function(data){var rates=data&&data.rates?data.rates:{};var usdTry=rates.TRY;var eurTry=rates.TRY&&rates.EUR?(rates.TRY/rates.EUR):null;var gbpTry=rates.TRY&&rates.GBP?(rates.TRY/rates.GBP):null;text('imdeksUsdTry',fmtRate(usdTry));text('imdeksEurTry',fmtRate(eurTry));text('imdeksGbpTry',fmtRate(gbpTry));text('imdeksCurrencyDate',data&&data.time_last_update_utc?'Güncelleme: '+new Date(data.time_last_update_utc).toLocaleDateString('tr-TR'):'Güncel kurlar');}).catch(function(){text('imdeksCurrencyDate','Döviz bilgisi alınamadı');});}
    if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',function(){detectWeather();loadCurrency();});}else{detectWeather();loadCurrency();}
})();
